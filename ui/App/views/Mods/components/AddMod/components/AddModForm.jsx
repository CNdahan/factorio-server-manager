...
const LinkModPortal = () => {
    ...
    return <a href="https://mods.factorio.com" target="_blank" className="px-2 text-blue hover:text-blue-light">{t('mods.modPortal')}
    ...
    return (
        <form onSubmit={handleSubmit(openSelectVersionModal)}>
            ...
            <div className="mb-4 relative" >
                <Label text={t('mods.mod')} htmlFor="mod"/>
                { typeof fuse !== "undefined"
                    ...
                    : <div className="border border-gray-medium w-full py-2 px-3 text-white">
                        <FontAwesomeIcon icon={faSpinner} spin={true}/> {t('mods.loadingList')} <LinkModPortal/>
                    </div>
                }
            ...
            <Button isDisabled={selectedMod === null} isSubmit={true} onClick={() => setIsModalOpen(true)} className="mr-2">{t('common.install')}</Button>
            <Button onClick={logout} type="danger" className="mr-2">{t('nav.logout')}</Button>
            <LinkModPortal/>
        </form>
    )
}
